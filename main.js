//Input Data
	...
	var largZone=10; //Number of column blocks
	var hautZone=10; //Number of line blocks
		...
		for(ligne=0;ligne<pdm;ligne++) //for each line in the current block
		{
			for(col=0;col<pdm;col++) //for each column in the current block
			{
				if(col==0 && ligne==0)
					...
					relief[bl*pdm+ligne][bc*pdm+col]=Math.abs(relief[bl*pdm+ligne-1][bc*pdm+col]+Math.floor(Math.random()*3)-1);
				else if(ligne==0)
					relief[bl*pdm+ligne][bc*pdm+col]=Math.abs(relief[bl*pdm+ligne][bc*pdm+col-1]+Math.floor(Math.random()*3)-1);
				else relief[bl*pdm+ligne][bc*pdm+col]=Math.abs((relief[bl*pdm+ligne-1][bc*pdm+col]+relief[bl*pdm+ligne-1][bc*pdm+col])/2+Math.floor(Math.random()*3)-1);
			}
		}
